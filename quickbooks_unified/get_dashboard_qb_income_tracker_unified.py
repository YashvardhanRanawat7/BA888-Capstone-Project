...
class AgedReceivableProcessor(PLProcessor):
    def process_data(self):
        try:
            ...
            self.add_graph("Aged Payables Aging Breakdown", "bar chart", barChartAccountsPayable, options=barChartOptions)
        except Exception as e:
...
class TotalRevenueInvoiceProcessor(PLProcessor):
    def process_data(self):
        try:
            ...
            #find unpaid invoice by customer
...
def get_dashboard_qb_income_tracker_unified(dashboard_id, df_path):
    try:
        ...
        print("df_contact", df_contact)
        df_invoice =  pd.DataFrame(df["invoice"])
        print("df_invoice", df_invoice)
